...
TARGET_COLUMN = "Daily new confirmed deaths due to COVID-19 per million people (rolling 7-day average, right-aligned)"
...
def line(data, period=None):
    ...
    fig = px.line(
        ...
        color='Entity',
        labels={'Day': 'Date', y_column: 'Daily Deaths'},
        title=f'<b>Daily Deaths {period} per 1 000 000</b> <br><i>(7-day average)</i>'
    )
    ...
    fig.update_traces(
        hovertemplate=f'Country: %{{customdata[1]}}<br>Date: %{{customdata[0]}}<br>Deaths number: %{{y:,.0f}}<extra></extra>'
    )
    ...
    file_path = f'../plots/Deaths{period}_multi.html'
